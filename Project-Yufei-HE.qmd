...
#### 1.2.Lasso Regression Results

The Lasso selection results indicate that co2_per_unit_energy has a relatively small impact on the changes in energy structure and is thus ignored in the subsequent regression process. The coefficients for the Lasso regression results are presented in Table .
...
The results of the LASSO regression provide a simple and preliminary interpretation, with the primary outcome being variable selection. More detailed, robust, and precise results will be reflected in the subsequent sections.
...
It can be found that the coefficients of the variables in the elastic net regression have the same sign as the coefficients of the variables in the Lasso regression, and they represent the same economic effects. However, this method screens out many variables, particularly control variables that are negatively correlated with the dependent variable. If the selection results of the elastic net are chosen, subsequent regression and other analyses will become unreliable. This indicates an excessive sensitivity of the elastic net method.

Compared to elastic net regression, LASSO regression with a regularization coefficient of 1 does not lose variables due to excessive sensitivity. This indicates that LASSO regression is relatively reliable.

The traditional mean regression of ols carving out the whole does not deal well with idiosyncrasies and takes into account the full picture of the conditional distribution. Quantile Regression is used here to minimize these shortcomings. The basic idea of quantile regression is to split the data into multiple quantile points according to the dependent variable, and study the regression effect of different quantile points, which is essentially a weighted regression method. In addition, quantile regression estimation is estimated by the weighted lest absolute deviation and (weighted lest absolute deviation (WLAD)) method, which is usually not affected by outlier points, and the results are more robust.

We select  25%, 50%, 75% of the quartiles for regression. The results are shown in Table.
...
qr_models <- lapply(taus, function(tau) {
  rq(co2 ~ ., data = data, tau = tau)
})

lapply(qr_models, summary)
```

(Uncertain yet). As can be seen from the table, co2_per_capital has a more significant effect and larger regression coefficients in areas with higher co2 emissions. This may be due to the fact that in areas with lower emission levels, CO2 emissions may be more influenced by factors such as infrastructure and industrial structure, whereas in high emission areas, where industrial activities are more intensive, the effect is greater.

Methane has a larger absolute value of the coefficient and a positive coefficient on the 25% and 75% segments. methane_per_capital has a larger absolute value of the coefficient and a negative coefficient on the 25% and 75% segments.

The effect of methane on CO2 emissions can be explained in a manner similar to the Kuznets curve. Energy production and agricultural activities emit large amounts of methane, while emitting carbon dioxide. Areas with lower CO2 emissions have lower levels of industrialization and more agricultural activities, and therefore have a significant impact; similarly, areas with higher CO2 emissions have more industrial activities and greater energy development and consumption.
